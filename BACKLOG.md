...
What this format knows it does not yet do.

Every entry names the event that would revive it. Nothing here is a promise
and nothing is scheduled: an item is designed when something real demands it,
on the rule that a field earns its place by a demonstrated need rather than by
symmetry or by anticipation. Items leave this file when their trigger fires,
either into the specification or into the design history's declined register

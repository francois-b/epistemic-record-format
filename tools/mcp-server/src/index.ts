...
export function buildServer(ws: Workspace): McpServer {
  ...
  server.registerTool("erf_corpus_check", { title: "Check the corpus", description: "Load and validate: violations by requirement, the quote check over every atom, dispositions, unbacked claims, captured-but-uncited sources, broken anchors. Use when asked where things stand.", inputSchema: { corpus: corpusArg } }, on("erf_corpus_list", (c) => T.corpusCheck(c)));

...
export function buildServer(ws: Workspace): McpServer {
  ...
  server.registerPrompt("work-the-flags", { title: "Work the flags", description: "Take the open flags one at a time: decompose each flagged passage into claims and wait for a ruling.", argsSchema: {} }, () => ({ messages: [{ role: "user", content: { type: "text", text: `List the open flags with erf_flags. Take the first: decompose its passage into every checkable assertion (id, kind, the claim as one sentence no stronger than the passage, what would back it, anchor words), merge assertions that are one claim, and stop for my ruling. After I rule, mint the accepted claims and bind the passage with erf_narrative_bind, which resolves the flag; then the next flag.` } }] }));

...
export function buildServer(ws: Workspace): McpServer {
  ...
  server.registerTool("erf_flags", { title: "List flags", description: "The passages flagged for backing, each with the passage text, so they can be decomposed one by one. Open flags by default.", inputSchema: { corpus: corpusArg, narrative: z.string().optional(), all: z.boolean().optional() } }, on("erf_flags", (c, a) => T.flags(c, a)));

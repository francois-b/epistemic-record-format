...
export function buildServer(ws: Workspace): McpServer {
  ...
  server.registerPrompt("work-the-flags", { title: "Work the flags", description: "Take the open flags one at a time: decompose each flagged passage into claims, wait for a ruling, then do what the flag's research field asked for.", argsSchema: {} }, () => ({ messages: [{ role: "user", content: { type: "text", text: `List the open flags with erf_flags. Skip any flag another worker has taken, and take the first one that is free with erf_flag_take before you start on it; someone else may be working the same queue. Then: decompose its scope, the span marked «so» in the listing, into every checkable assertion (id, kind, the claim as one sentence no stronger than the text, what would back it, anchor words); the passage around the span is context only, and what the rest of it asserts goes under "also in this passage, not flagged" and waits for its own flag. Merge assertions that are one claim, and stop for my ruling. A flag whose research is survey runs the other way round: research the span first (log every search with what it was for, capture each page you read, mint atoms from verbatim quotes, record the survey with its coverage bounds and notable results), then propose the claims the survey supports, scoped to the span, and stop for my ruling. After I rule, mint the accepted claims and bind the passage with erf_narrative_bind, which resolves the flag. Then do what that flag's research field asked for: mint or survey, stop there; back, back each accepted observation before binding (log every search with what it was for, capture each page you read, mint atoms from verbatim quotes, and record a survey for each backed observation, gap or not); opposite, do all of back and also state the strongest case against each claim, with where that evidence would be found, before I stand on anything. Then the next flag.` } }] }));

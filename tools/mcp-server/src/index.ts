...
export function buildServer(ws: Workspace): McpServer {
  ...
  const on = <A extends { corpus?: string }>(fn: (c: ReturnType<typeof resolveCorpus>, a: A) => T.Result | Promise<T.Result>) =>
    async (a: A) => { try { const c = resolveCorpus(ws, a.corpus); return text(c.id, await fn(c, a)); } catch (e) { return refused(e); } };

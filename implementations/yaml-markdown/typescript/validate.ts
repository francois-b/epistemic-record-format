/**
 * Validating a corpus of the YAML/Markdown serialization: every document
 * against the schema (ERF-73), the file-level rules (ERF-65/66/67), and the
 * invariants that span records (section 6: ids resolve, no cycles, quotes
 * checkable, stamps ordered). `loadCorpus` is the entry: it reads (read.ts),
 * validates, materializes ERF-56's total lists, and returns the model plus
 * every finding. The readings computed over the model are compute.ts.
 */
...
import { splitFrontmatter, walkFiles, fileType, listDir, YAML_OPTS, bindingRe, bindingCandidates, unescapeAnchor, splitDocument } from "./read.ts";
export * from "./read.ts";

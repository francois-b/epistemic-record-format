# @epistemic-record-format/yaml-markdown
...
```
npm install @epistemic-record-format/yaml-markdown
npx erf-check <corpus-dir>
```

```ts
import { loadCorpus, disposition, quoteCheck, SPEC_VERSION } from "@epistemic-record-format/yaml-markdown";
...
The schema this build implements ships with the package: `import schema from "@epistemic-record-format/yaml-markdown/schema"` (JSON Schema 2020-12, the same bytes as `schema/erf.schema.json` at the spec version in `SPEC_VERSION`). Package versions are the implementation's own; `package.json` names the spec version it implements under `erf.spec_version`.

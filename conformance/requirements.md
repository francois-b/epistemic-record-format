...
| [`ERF-2`](../SPEC.md#erf-2) | A raw file is immutable: a revision arriving later MUST be a new source, never an overwrite. |  | untestable by design: how a capture is made and dated is a producer practice, not a property of a loaded record |
| [`ERF-4`](../SPEC.md#erf-4) | Every atom MUST name a source that exists in the corpus's source list. | Corpus | `fixtures/invalid/atom-absent-from-mapping` |
...
| [`ERF-69`](../SPEC.md#erf-69) | A source's normalized text MAY be an excerpt of the work rather than a whole copy, and MUST then record who selected the passage and when (`excerpt`). |  | `fixtures/valid/excerpt-in-passages`, `fixtures/invalid/quote-crosses-an-excerpt-gap` |
| [`ERF-70`](../SPEC.md#erf-70) | Where normalized text was produced from a raw file in another format, the source MUST name the extracting tool and its… |  | `fixtures/invalid/pdf-without-extraction` |
...
| [`ERF-6`](../SPEC.md#erf-6) | The `quote` MUST be verbatim from the source's normalized text. | Producer | `quote-check` |
| [`ERF-8`](../SPEC.md#erf-8) | When `citation` is present it is canonical: it MUST carry everything the rendered `citation_text` string shows… |  | untestable by design: whether a rendered citation carries everything its structured form holds is a judgment about bibliographic completeness |
| [`ERF-9`](../SPEC.md#erf-9) | `source_quality` MUST grade one axis, how much weight the attester's word carries for the fact the finding conveys, the… |  | untestable by design: which of two inputs governs a source's grade is an assessment, not a computation |
| [`ERF-10`](../SPEC.md#erf-10) | The grade MUST be assessed against the substance the finding conveys, not against the bare fact that someone uttered it. |  | untestable by design: whether a finding's subject is discourse itself is a reading of the finding |
...
| [`ERF-14`](../SPEC.md#erf-14) | `as_of_date`, where present, MUST record the date the fact is true of, at the precision the source gave and no finer: a year, a year and month, or a full date. |  | `schema` |
| [`ERF-15`](../SPEC.md#erf-15) | References MUST be bare ids and MUST NOT encode location. |  | `fixtures/invalid/reference-encodes-location` |
| [`ERF-17`](../SPEC.md#erf-17) | A record's `corpus` MUST name the corpus the deployment declares. |  | `fixtures/valid/minimal`, `schema` |
| [`ERF-18`](../SPEC.md#erf-18) | `title` MUST state the claim; it is the normative statement. |  | untestable by design: that a title states the claim is the authoring act itself; the mechanical half was ERF-46, retired 2026-08-24 into this guidance |
...
| [`ERF-23`](../SPEC.md#erf-23) | Evidence MUST live on the claim, in both directions: `atoms_for` and `atoms_against`. |  | untestable by design: modelling counter-evidence as a rival claim is a shape the format asks an author not to build |
| [`ERF-24`](../SPEC.md#erf-24) | The backing audit MUST ask the question the epistemic kind sets, because the kind is the backing contract. |  | untestable by design: whether atoms jointly entail a statement is the audit judgment itself |
| [`ERF-25`](../SPEC.md#erf-25) | A universal negative, a claim of the form "no shipped tool does X", MUST be audited as scoped rather than as proved. |  | untestable by design: whether a claim is a universal negative is a reading of the claim |
| [`ERF-26`](../SPEC.md#erf-26) | Each search act MUST name its concrete instrument in `tool` and its `query` in that instrument's own terms, and MAY name the `scope` that applied. |  | uncovered: no fixture asserts a search act naming a category rather than an instrument |
| [`ERF-27`](../SPEC.md#erf-27) | `hits_reported` MUST record each act's yield as the instrument reported it, as text, and MUST NOT state precision the instrument did not give. |  | uncovered: no fixture asserts hits_reported rejects invented precision |
| [`ERF-28`](../SPEC.md#erf-28) | What a survey conducted is immutable: `searches` and each act's reported yield MUST NOT change after the fact, because… |  | uncovered: no fixture covers survey immutability or the title stating what was sought |
...
| [`ERF-35`](../SPEC.md#erf-35) | A reference asserting a *current* relationship MUST resolve within the deployment, the corpora read and cited together… |  | `fixtures/invalid/prior-survey-dangling`, `fixtures/valid/evidence-at-stance-outlives-atom`, `fixtures/invalid/atoms-for-names-a-survey` |
...
| [`ERF-40`](../SPEC.md#erf-40) | Standings MUST be append-only; an edit or deletion of an existing entry is a violation, verified against the substrate's history. |  | untestable by design: verified against substrate history, which a loaded corpus does not carry |
...
| [`ERF-61`](../SPEC.md#erf-61) | A MAJOR increment of `spec_version` MUST mean that records of the previous major are unreadable or read with changed… |  | `schema` |
...
| [`ERF-63`](../SPEC.md#erf-63) | A substrate MAY be anything that preserves records, ids, attribution, and an edit history sufficient to verify `ERF-40`. |  | untestable by design: a permission, satisfiable by any substrate |
...
| [`ERF-66`](../bindings/yaml-markdown.md#erf-66) | A record's frontmatter MUST NOT contain a duplicate key, an anchor, an alias, or an explicit tag. |  | `fixtures/invalid/frontmatter-duplicate-key`, `fixtures/invalid/frontmatter-anchor` |

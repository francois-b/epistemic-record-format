...
2. **Decompose.** For a flagged passage (the exact words the person selected), the LLM lists every assertion it makes (a flag on one sentence is not a flag on its paragraph; the paragraph around it is context, and what the rest of it asserts waits for its own flag), each typed by what would settle it (`observation`, `argument`, `commitment`, `bet`), with the claim stated no stronger than the passage states it, what would back it, and the anchor words. Assertions that are one claim are merged. Nothing is written. *Tooling: the `decompose-passage` prompt; `erf_flags` lists what is waiting.*

3. **Rule.** The person accepts, narrows, or drops each proposal, on a card that shows each with its evidence in full: the verbatim quotes, where each came from (the source page, the held text, the page of a PDF), what would settle the claim, and the worker's remark. A narrowing is the ordinary outcome: the title is edited in place and minted as edited. Nothing about a claim exists before this step but the proposal; the ruling is what writes it. *Tooling: the worker puts the set with `erf_propose`, which renders the card in the conversation; each action on the card is `erf_proposal_rule` (accepted and narrowed mint the claim with the atoms as ruled; dropped records the drop); "bind and finish" is `erf_proposal_finish`. The set lives in `proposals.jsonl` at the corpus root, producer machinery like the flags. In chat, the worker says in a line that the proposals are on the card; a ruling made in words is carried out with the same tools.*

4. **Mint and bind.** The rulings wrote the accepted claims; finishing the set binds the passage to them with an instant-stamped marker, which resolves the flag (a set dropped whole resolves it without a binding). The one line that reports the rulings goes into the conversation from the card, so the loop continues in the same chat. *Tooling: `erf_proposal_finish`; underneath it `erf_claim_mint` and `erf_narrative_bind`, which stay the tools for a claim or a binding the person asks for in as many words; `erf_narrative_check` reports the binding as current.*

5. **Back.** For each observation, the LLM searches (each act logged with what it was for), captures the pages it reads (bytes held, digested, registered), picks verbatim quotes from the held text, and mints atoms; a paraphrase is refused. It records a survey for every observation it backed (what was sought, the coverage bounds, the notable results), gap or not: the survey is the review the person asked for, and the atoms hang off it. A flag that asked for a survey runs these steps first and proposes its claims after, scoped to the span, so the person decides what to claim knowing what was found; and it runs them as **recall, then verify**: before any search the LLM writes the sources it expects to exist for the span (author, title, year, what each would show, where it likely sits), marked as unverified recollection, then seeks each by name and logs its fate (held, unreachable with why, not found), widening to topic searches only after the named targets are settled. The survey lists those targets, so a later reader can tell absent from the literature from absent from this pass. Recollection is a hypothesis list, not knowledge: the capture confirms or kills each name. *Tooling: `erf_source_add`, which takes the search that led to the page (`found_by`, logged before the capture) and returns the passage around the phrase given as `find`, so the quote is chosen without reading the page again; `erf_atom_mint`, which takes every atom for that source in one call and reports each outcome in order; `erf_search_log` for a search that found nothing worth capturing; `erf_source_read` to re-read a source captured earlier; `erf_survey_record`, with `targets` for the sources sought and `for` as a list when the acts were logged under more than one question; the `survey-passage` prompt for the recall-then-verify order.*
...
- **A flag** is `{ts, narrative, anchor, span?, note?, research?, by, status, taken_by?, taken_ts?}` in `flags.jsonl` at the corpus root, append-only, resolved by the binding that covers its anchor. `span` is the whole selection with whitespace folded, kept when it is longer than the anchor: it is the flagged passage (what gets decomposed and backed), and the anchor is the few words that locate it. To the person it is always "the flagged passage"; `span` is a field name. `research` is what the flag asked for (`mint`: propose the claims and stop; `survey`: recall the expected sources, verify each by name, record the survey with its targets, then the claims it supports; `back`: claims, then evidence, then bind; `opposite`: back plus the case against) and defaults to `mint`. It is producer machinery, like the research log; the format does not see it.
...
- **A proposal set** is one worker's proposals for one flag, in `proposals.jsonl` at the corpus root: `{flag, ts, by, narrative, anchor, span?, research, survey?, summary?, proposals: [{id, title, epistemic_kind, atoms_for?, atoms_against?, settles?, note?}], rulings: {id: {ruling, ts, title?, claim?}}, status}`, `status` being open, ruled, or superseded (a later set for the same flag). A proposal names the id the claim would take; the claim exists only once the ruling mints it. Producer machinery; the format does not see it.

---
id: acx-93
type: atom
corpus: ai-capex
finding: 'SK hynix''s Q2 2026 results release reports 79.3187 trillion won in revenue, 60.5426 trillion won in operating profit (a 76% operating margin), and 93.9226 trillion won in net profit (a 118% net margin), which it characterizes as an all-time high quarterly performance.'
quote: 'SK hynix Inc. (or "the company", www.skhynix.com) announced today that it has recorded 79.3187 trillion won in revenues, 60.5426 trillion won in operating profit (with an operating margin of 76%), and 93.9226 trillion won in net profit (with a net margin of 118%), marking an all-time high quarterly performance.'
source: n-skhynix-q2-2026-results
source_quality: high
as_of_date: '2026-06-30'
limitations: 'A net margin (118%) exceeding the operating margin (76%) implies non-operating income not itemized in this excerpt; the release does not explain the gap.'
created: {timestamp: '2026-08-25', by: 'agent/claude-sonnet-5'}
finding_audit:
  - {auditor: gemini-3.5-flash, verdict: SUPPORTED, timestamp: '2026-08-25', protocol: capex-audit-v1}
  - {auditor: deepseek-v4-pro, verdict: SUPPORTED, timestamp: '2026-08-25', protocol: capex-audit-v1}
---

...
- **`@erf/validator`** ([`validator/yaml-markdown/typescript/`](validator/yaml-markdown/typescript/)) is the reference validator, as a library and as the `erf-check` command. It loads a corpus, validates every document against the schema, runs the quote check, and computes every reading the specification defines. Exit 1 on any violation.
